...
KLIST_ADMIN_RATE = (
    30,
    600,
)  # at most 30 admin-auth attempts / 600s / client IP (short PIN)
...
JOBS_ADMIN_RATE = (30, 600)  # at most 30 admin-auth attempts / 600s / client IP
# Separate from LOG_ACCESS_TOKEN / KLIST_ACCESS_TOKEN so the jobs-board admin token
# never unlocks the chat-log export or the klist viewer (one secret per surface).
JOBS_ACCESS_TOKEN = os.getenv("JOBS_ACCESS_TOKEN")
