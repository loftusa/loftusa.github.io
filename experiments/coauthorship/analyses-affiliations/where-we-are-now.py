...
assert sum(assigned_counts.values()) == 51     # 54 people minus the 4 with no current chapter
...
assert sum(metro_size.values()) + len(unknown) == 55
...
assert sum(len(p["all_current"]) for p in people) == 93        # raw current hats, fred's Harvard folded
