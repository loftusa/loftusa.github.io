...
assert sum(assigned_counts.values()) == 52     # 56 people minus the 4 with no current chapter
...
assert sum(metro_size.values()) + len(unknown) == 56
...
assert sum(len(p["all_current"]) for p in people) == 94        # raw current hats, fred's Harvard folded
